...
namespace InferencingSample
{
    ...
    {
        private SKColor ReadPixelColor(int Width,int Height, SKBitmap bitmap, int x, int y)
        {
        ...
        private SKColor GetPixelBilinear(float fx, float fy, int imgWidth,int imgHeight, SKBitmap bitmap)
        {
        ...
        public SKBitmap WarpAffineWithImage(int widthImage , int heightImage, SKBitmap image_bit, float[][] affineMatrix, int width = 224, int height = 224)
        {
            ...
            float[][] transformationMatrix = affineMatrix;
            ...
            {
                throw new Exception("Width and height must be 224, other transformations are not supported yet.");
            }
            ...
            for (int yTrans = 0; yTrans < height; ++yTrans)
            {
                for (int xTrans = 0; xTrans < width; ++xTrans)
                {
                    ...
                    SKColor pixel = GetPixelBilinear(xOrigin, yOrigin, widthImage, heightImage, image_bit);
