...
namespace InferencingSample
{
    public class CheckQuality
    {
        public List<List<int>> createGrayImage(SKBitmap originalBitmap)
        {
            ...
            for (int y = 0; y < originalBitmap.Height; y++)
            {
                ...
                for (int x = 0; x < originalBitmap.Width; x++)
                {
                    ...
                    int intensity = (int)Math.Clamp(Math.Round(color.Red * 0.229f + color.Green * 0.587f + color.Blue * 0.114f), 0, 255);
                    row.Add(intensity);
        ...
        public bool Brightness(List<List<int>> grayImage)
        {
            ...
            if (meanBrightness >= 80 && meanBrightness <= 300)
            {
                return false;
            }
            else
        ...
        public bool PredictIsBlurGrayLaplacian(
        ...
        private List<List<int>> ApplyLaplacian(List<List<int>> image)
        {
